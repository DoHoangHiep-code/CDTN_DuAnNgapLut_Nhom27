...
CREATE UNIQUE INDEX IF NOT EXISTS mv_latest_flood_predictions_pkey
  ON mv_latest_flood_predictions (prediction_id);
CREATE INDEX IF NOT EXISTS idx_mv_latest_fp_node_id
...
SELECT
  ...
  COUNT(*) AS node_count
FROM flood_predictions
GROUP BY time, date_only, hour, risk_level
ORDER BY time;

CREATE UNIQUE INDEX IF NOT EXISTS mv_global_risk_trend_pkey
  ON mv_global_risk_trend (time, risk_level);
