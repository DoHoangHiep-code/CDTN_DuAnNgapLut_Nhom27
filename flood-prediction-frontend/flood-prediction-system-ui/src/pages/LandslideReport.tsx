...
import { 
  ...
  MapPin, AlertTriangle, ShieldCheck, ChevronLeft, ChevronRight
} from 'lucide-react';
...
// Mock Data
const MOCK_DATA = [
  { id: 1, commune: 'Thị trấn Mù Cang Chải', district: 'Mù Cang Chải', province: 'Yên Bái', lat: 21.8562, lon: 104.0863, rain7d: 156, probability: 92, level: 'Nguy hiểm' },
  { id: 2, commune: 'Xã Púng Luông', district: 'Mù Cang Chải', province: 'Yên Bái', lat: 21.8315, lon: 104.1488, rain7d: 142, probability: 88, level: 'Nguy hiểm' },
  { id: 3, commune: 'Phường Phan Si Păng', district: 'Sa Pa', province: 'Lào Cai', lat: 22.3364, lon: 103.8438, rain7d: 215, probability: 85, level: 'Nguy hiểm' },
  { id: 4, commune: 'Xã Tả Van', district: 'Sa Pa', province: 'Lào Cai', lat: 22.2886, lon: 103.8967, rain7d: 180, probability: 78, level: 'Cảnh báo' },
  { id: 5, commune: 'Xã Nậm Pung', district: 'Bát Xát', province: 'Lào Cai', lat: 22.5833, lon: 103.7167, rain7d: 120, probability: 65, level: 'Cảnh báo' },
  { id: 6, commune: 'Xã Y Tý', district: 'Bát Xát', province: 'Lào Cai', lat: 22.6500, lon: 103.6167, rain7d: 95, probability: 45, level: 'An toàn' },
  { id: 7, commune: 'Xã Pà Cò', district: 'Mai Châu', province: 'Hòa Bình', lat: 20.7333, lon: 104.9333, rain7d: 110, probability: 40, level: 'An toàn' },
  { id: 8, commune: 'Xã Hang Kia', district: 'Mai Châu', province: 'Hòa Bình', lat: 20.7500, lon: 104.9000, rain7d: 85, probability: 30, level: 'An toàn' },
];
...
export function LandslideReportPage() {
  ...
  const provinces = ['Tất cả', ...Array.from(new Set(MOCK_DATA.map(d => d.province)))];
  ...
  const topHotspots = [...MOCK_DATA].sort((a, b) => b.probability - a.probability).slice(0, 4);
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Dữ liệu dự báo sạt lở lưới (Grid Nodes)
            </p>
          </div>
        </div>
      </div>
            ...
            <div key={idx} className="relative flex flex-col overflow-hidden rounded-2xl border border-rose-400/80 bg-gradient-to-b from-rose-100 to-white shadow-rose-200/50 shadow-lg ring-2 ring-rose-300/50 dark:border-rose-600 dark:from-rose-900/40 dark:to-slate-900 p-4">
              <div className="flex justify-between items-start mb-2">
                <div>
                  ...
                  <div className="text-xs text-slate-500">{item.district}, {item.province}</div>
                </div>
          ...
          <div className="flex flex-col gap-3 border-b border-slate-100 px-5 py-4 dark:border-slate-800 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <div className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                Đang hiển thị {filteredData.length} điểm rủi ro / Tổng số 425,180 điểm
              </div>
            </div>
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <div className="text-[10px] text-slate-500">{item.district}, {item.province}</div>
                        </div>
                      </div>
                    </td>
