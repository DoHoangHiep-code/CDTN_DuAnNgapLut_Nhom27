...
export function DashboardPage() {
  ...
  // ✅ Must be at the top — before any early returns (Rules of Hooks)
    ...
    const t = setTimeout(() => {
      if (search.trim()) {
        ...
        import('../../../../services/api').then(({ getDashboardAutocomplete }) => {
          getDashboardAutocomplete(search)
            .then(data => {
              const unique = Array.from(new Map(data.map(item => [item.location_name, item])).values());
    ...
    <div className="space-y-5">
      ...
      {meta?.search && meta.resolvedNodes.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {Array.from(new Set(meta.resolvedNodes.map(n => n.name))).map((uniqueName, idx) => (
            <span key={idx} className="rounded-full border border-slate-200 bg-white px-2.5 py-0.5 text-[11px] text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300">
              {uniqueName}
            </span>
          ))}
        </div>
