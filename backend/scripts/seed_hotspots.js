...
async function main() {
  ...
  try {
    ...
    console.error('[DB] ❌ Không kết nối được Supabase:', err.message)
    process.exit(1)
  }

  // Sync schema (alter:true) để cột location_name được tạo trên Supabase
  try {
    await sequelize.sync({ alter: true })
    console.log('[DB] ✅ Schema đã sync (alter:true).\n')
  } catch (err) {
  ...
  try {
    ...
    console.log(`\n✅ THÀNH CÔNG! Đã upsert ${results.length} GridNode vào Supabase.`)
    console.log('   Bảng grid_nodes đã được điền đầy đủ 39 điểm ngập thực tế Hà Nội.')
    console.log('\n📌 Bước tiếp theo:')
    console.log('   1. Mở Supabase Dashboard → Table Editor → grid_nodes để kiểm tra.')
