...
import { getFloodPrediction, getWeather, getWeather7Days, getWeatherForecast24h } from '../../../services/api'
...
function toVNHour(iso: string): number {
  ...
  return hourPart ? parseInt(hourPart.value, 10) : new Date(iso).getHours()
}
...
const KIND_CONFIG: Record<WeatherKind, {
  ...
  rain: {
    cardBg: 'border-sky-300 bg-gradient-to-b from-sky-50 to-white shadow-sky-100 shadow-md ring-1 ring-sky-200 dark:border-sky-700 dark:from-sky-900/20 dark:to-slate-900',
    gradient: 'from-sky-500 via-blue-500 to-indigo-600',
    glassRing: 'ring-sky-300/40',
  ...
  sun: {
    cardBg: 'border-amber-200 bg-gradient-to-b from-amber-50 to-white shadow-sm hover:border-amber-300 dark:border-amber-700/50 dark:from-amber-900/10 dark:to-slate-900',
    gradient: 'from-amber-400 via-orange-400 to-rose-500',
    glassRing: 'ring-amber-300/30',
  ...
  flood: {
    cardBg: 'border-rose-400/80 bg-gradient-to-b from-rose-100 to-white shadow-rose-200/50 shadow-lg ring-2 ring-rose-300/50 dark:border-rose-600 dark:from-rose-900/40 dark:to-slate-900',
...
// ── WeatherForecastCard ───────────────────────────────────────────────
function WeatherForecastCard({ d, isToday }: {
  d: any
  isToday?: boolean
}) {
  ...
  const owmIconUrl = d.icon ? `https://openweathermap.org/img/wn/${d.icon}@2x.png` : null
  ...
  return (
    <div className={cn(
      ...
      {isToday && (
        <div className="absolute top-0 right-0 bg-sky-500 text-white text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-bl-lg z-10 shadow-sm">
          Hôm nay
        </div>
      )}
      <div className="p-4 space-y-3">
        <div className="flex items-start justify-between">
          <div className="space-y-0.5">
            ...
            <span className={cn('inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide', cfg.badgeColor)}>
              {owmIconUrl ? <img src={owmIconUrl} alt={d.description} className="h-4 w-4 drop-shadow-sm" /> : <Icon className="h-3 w-3" />}
              {d.description || cfg.label}
            </span>
          </div>
          <div className={cn('grid h-10 w-10 flex-shrink-0 place-items-center rounded-xl ring-2', cfg.glassRing, cfg.iconBg)}>
            {owmIconUrl ? (
              <img src={owmIconUrl} alt={d.description} className="h-8 w-8 drop-shadow-md" />
            ) : (
              <Icon className={cn('h-6 w-6 stroke-[2.5]', cfg.iconColor)} />
            )}
          </div>
        </div>
...
function HourlyForecast({ forecast24h }: {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900 relative">
      <div className="flex items-center justify-between border-b border-slate-100 px-5 py-3.5 dark:border-slate-800">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-xs text-slate-400">Cuộn để xem quá khứ / tương lai</div>
          </div>
        </div>
      ...
      <div ref={scrollContainerRef} className="flex gap-2 overflow-x-auto px-4 py-3 scrollbar-hide">
        {forecast24h.map((p) => {
          ...
          let hourBg = ''
          if (isNow) {
            hourBg = 'border-sky-400 bg-gradient-to-b from-sky-500 to-indigo-600 text-white shadow-lg shadow-sky-200 dark:shadow-sky-900/40 scale-105 hover:border-sky-300'
          } else if (rainKind === 'flood') {
            hourBg = 'border-rose-200 bg-gradient-to-b from-rose-50 to-white text-slate-700 shadow-sm hover:border-rose-300 dark:border-rose-800 dark:bg-rose-900/20 dark:text-slate-200'
          } else if (rainKind === 'rain') {
            hourBg = 'border-sky-200 bg-gradient-to-b from-sky-50 to-white text-slate-700 shadow-sm hover:border-sky-300 dark:border-sky-800 dark:bg-sky-900/20 dark:text-slate-200'
          } else {
            hourBg = 'border-amber-100 bg-gradient-to-b from-amber-50 to-white text-slate-700 shadow-sm hover:border-amber-200 dark:border-amber-800 dark:bg-amber-900/10 dark:text-slate-200'
          }
          ...
          return (
            <div
              ...
              className={cn(
                'flex min-w-[68px] flex-shrink-0 flex-col items-center gap-1.5 rounded-2xl border px-2.5 py-3 transition-all hover:scale-105 hover:shadow-md cursor-pointer',
                hourBg
              )}
            >
              {/* Giờ */}
              ...
              {/* Icon thời tiết */}
              <RainIcon className={cn('h-5 w-5 stroke-[2.5]', isNow ? 'text-white' : rainKind === 'rain' ? 'text-sky-500' : rainKind === 'flood' ? 'text-rose-600' : 'text-amber-500')} />
      ...
      <div className="flex items-center gap-2 border-t border-slate-100 px-5 py-2.5 dark:border-slate-800">
        ...
        <span className="text-[11px] text-slate-400 dark:text-slate-500">
          Chỉ số nguy cơ trung bình: <span className="font-bold text-slate-600 dark:text-slate-300">
            {(total > 0 ? districts.reduce((s, d) => s + d.flood_depth_cm, 0) / total : 0).toFixed(1)}cm
          </span>
        </span>
      </div>
    </div>
...
function FloodWeatherPage() {
  ...
  const forecast7d = useAsync(
    () => getWeather7Days(...forecast7dParams),
    [forecast7dParams],
  )
  ...
  const f7dData = forecast7d.data as any[] | null
  const forecast7dData = f7dData && f7dData.length > 0
    ? f7dData.map((d: any) => ({
        dateIso: d.dateIso,
        minTempC: d.minTempC,
        maxTempC: d.maxTempC,
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="lg:col-span-8 flex flex-col gap-2" style={{ height: 520 }}>
          ...
          <div className="min-h-0 flex-1 overflow-hidden rounded-2xl ring-1 ring-slate-200 shadow-sm dark:ring-slate-700">
            <MiniFloodMap
              ...
              searchPopupContent={
                selectedLocation ? (
                  <div className="min-w-[120px] p-1 space-y-1.5">
                    <div className="text-[13px] font-extrabold text-slate-800">{selectedLocation.name}</div>
                    <div className="flex items-center justify-between text-xs text-slate-600">
                      <span>Nhiệt độ:</span>
                      <span className="font-bold">{current.temperatureC}°C</span>
                    </div>
                    <div className="flex items-center justify-between text-xs text-slate-600">
                      <span>Lượng mưa:</span>
                      <span className="font-bold">{current.rainfallMm} mm</span>
                    </div>
                  </div>
                ) : null
              }
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
        ...
        <RiskOverview districts={floodData.data?.districts || []} />
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
        ...
        >
          {forecast7dData.map((d: any) => (
            <div key={d.dateIso} className="w-48 min-w-[180px] shrink-0">
              <WeatherForecastCard d={d} isToday={d.dateIso === new Date().toISOString().slice(0, 10)} />
            </div>
          ))}
        </div>
      </div>
