class ReportsController {
  ...
  async getHotspots(req, res, next) {
    try {
      const data = await this.reportsService.getHotspots()
      return res.status(200).json({ success: true, data })
    } catch (err) {
      console.error('[ReportsController] Error getHotspots:', err)
  ...
  async list(req, res, next) {
    try {
      ...
      const mapped = result.rows.map((r) => ({
        ...
        userFullName: r.User?.full_name ?? null,
        locationName: r.GridNode?.location_name ?? `Node ${r.node_id}`,
        districtName: r.GridNode?.district_name ?? '',
      }))
  ...
  async createActualFloodReport(req, res, next) {
    try {
      ...
      // 2. Tìm node_id gần nhất bằng ST_Distance
      const { GridNode } = require('../../../models')
      const { Sequelize } = require('sequelize')
      const nearestNode = await GridNode.findOne({
        attributes: ['node_id'],
        order: [
          [Sequelize.fn('ST_Distance', Sequelize.col('geom'), Sequelize.fn('ST_SetSRID', Sequelize.fn('ST_MakePoint', longitude, latitude), 4326)), 'ASC']
        ],
        raw: true
      })
      const node_id = nearestNode?.node_id || null

      // 3. Lưu vào DB qua service
      const created = await this.reportsService.create({
        userId,
        latitude,
        longitude,
