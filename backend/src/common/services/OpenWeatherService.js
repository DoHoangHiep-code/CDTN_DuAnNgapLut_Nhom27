...
/**
 * Lấy dự báo thời tiết theo ngày (lên đến 16 ngày) từ OWM.
 * Sử dụng cho Live UI (7 ngày tới), không lưu vào DB.
 ...
 * @returns {Promise<Array<object>|null>}
 */
async function getOWMDailyForecast(lat, lon, days = 7) {
  if (!_hasValidKey()) {
    console.warn('[OWM/Daily] OPENWEATHER_API_KEY chưa được cấu hình trong .env')
    return null
  }
  ...
  try {
    ...
    return list.map((item) => {
      // timeUtc (timestamp là lúc 12:00 trưa của ngày đó theo UTC)
      const dateIso = new Date(item.dt * 1000).toISOString().slice(0, 10)
      return {
        dateIso,
        minTempC:       Math.round((item.temp?.min ?? 26) * 10) / 10,
        maxTempC:       Math.round((item.temp?.max ?? 32) * 10) / 10,
        rainfallMm:     Math.round((item.rain ?? 0) * 10) / 10,
        humidityPct:    Math.round(item.humidity ?? 70),
        icon:           item.weather?.[0]?.icon ?? '',
        description:    item.weather?.[0]?.description ?? '',
      }
    })
  } catch (err) {
    _handleAxiosError(err, 'DailyForecast')
    return null
  }
}

module.exports = { getWeatherByCoords, getOWMForecast5d, getOWMHourlyForecast4d, aggregateToDaily, getOWMOneCall, getOWMDailyForecast }
