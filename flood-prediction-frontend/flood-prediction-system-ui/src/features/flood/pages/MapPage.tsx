  ...
  return (
    <>
      {clusters.map((f) => {
        ...
        if (isCluster) {
          if (!showFloodMarkers) return null
        ...
        return (
          <Fragment key={p.id}>
            ...
            {showFloodMarkers && (
              <Marker position={[lat, lng]} icon={markerIcon} eventHandlers={{ click: () => onSelectPoint(p) }}>
...
import { LandslidePage } from '../../landslide/pages/LandslidePage'
...
export function MapPage() {
  ...
  if (mode === 'landslide') {
    return <LandslidePage />
  }
  return <FloodMapContent />
}

function FloodMapContent() {
  const { t } = useTranslation()
  const { showRiskOverlay, showFloodMarkers, mapStyle, showFloodDepth, showWeatherStats } = useSettings()
  ...
  // Chỉ hiển thị điểm có nguy cơ (không phải safe)
  const floodPoints = useMemo(
    () => bboxData.districts.filter((d) => d.risk !== 'safe'),
    [bboxData.districts]
  )
  // Auto-hide empty state sau 12 giây
  const [showEmptyBanner, setShowEmptyBanner] = useState(false)
  useEffect(() => {
    if (!isFetchingBbox && bboxData.districts.length === 0) {
      setShowEmptyBanner(true)
      const t = setTimeout(() => setShowEmptyBanner(false), 12000)
      return () => clearTimeout(t)
    } else {
      setShowEmptyBanner(false)
    }
  }, [isFetchingBbox, bboxData.districts.length])
      ...
      <div className="grid grid-cols-12 gap-4">
        <div className="col-span-12 overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 lg:col-span-8">
          <div className="relative">
            ...
            {/* ── Empty State: Auto-dismiss, góc dưới phải bản đồ ── */}
            {showEmptyBanner && (
              <div className="absolute bottom-14 left-4 z-[1000] flex items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50/95 px-3 py-2 shadow-md backdrop-blur dark:border-emerald-800 dark:bg-emerald-950/90 animate-fade-in">
                <span className="text-base">✅</span>
                <div>
                  <div className="text-xs font-bold text-emerald-800 dark:text-emerald-200">Không có nguy cơ ngập</div>
                  <div className="text-[10px] text-emerald-600 dark:text-emerald-400">Khu vực hiện tại đang an toàn</div>
                </div>
              </div>
            )}
            ...
            >
              ...
              <FloodClustersLayer
                ...
                showFloodMarkers={showFloodMarkers}
              ...
              {selectedNodeData && (
                <Popup
                  ...
                  autoClose={false}
                  closeOnClick={false}
                  eventHandlers={{ remove: () => setTimeout(() => setSelectedNodeData(null), 0) }}
                >
