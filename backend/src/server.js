...
const { ensureTables } = require('./db/ensureTables')
...
app.use(
  cors({
    origin: ['http://localhost:5173', 'http://127.0.0.1:5173', 'http://localhost:5174', 'http://127.0.0.1:5174'], // Cho phép Vite dev server
...
async function bootstrapAndStart() {
  try {
    ...
    // 2) Đảm bảo các bảng thiếu được tạo trước khi Cronjobs chạy
    await ensureTables()
    console.log('[DB] Đã kiểm tra và tạo bảng thiếu nếu cần.')
