...
import type { ReportsRow } from '../../../utils/types'
...
function HotspotsCards() {
  ...
  const loading = hotspotsAsync.loading

  if (loading && !data.length) {
    return (
      <div className="space-y-3">
        <h3 className="text-sm font-extrabold text-slate-800 dark:text-slate-100">Giám sát Khu vực Trọng điểm</h3>
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {data.map((item: any) => {
          const isFlood = item.floodDepth > 0
          const isRain = !isFlood && item.rain > 0
          ...
          if (isFlood) {
            cardBg = 'border-rose-400/80 bg-gradient-to-b from-rose-100 to-white shadow-rose-200/50 shadow-lg ring-2 ring-rose-300/50 dark:border-rose-600 dark:from-rose-900/40 dark:to-slate-900 scale-[1.02]'
            ringColor = 'ring-rose-400/30'
            iconBg = 'bg-rose-200 dark:bg-rose-800'
            iconColor = 'text-rose-600 dark:text-rose-400'
            statusBadge = 'bg-rose-100 text-rose-700 dark:bg-rose-900/50 dark:text-rose-300'
            ...
            statusBadge = 'bg-sky-100 text-sky-700 dark:bg-sky-900/50 dark:text-sky-300'
            ...
            statusBadge = 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-300'
          ...
          return (
            <div key={item.name} className={cn(
              ...
              {item.is_fallback && (
                <div className="absolute top-0 right-0 bg-violet-500/90 backdrop-blur-sm text-white text-[8px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-bl-lg z-10 shadow-sm flex items-center gap-1">
