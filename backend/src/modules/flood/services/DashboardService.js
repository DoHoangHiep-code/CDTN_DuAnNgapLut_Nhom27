...
class DashboardService {
  ...
  async getDashboard({ hours = 24, search = '' } = {}) {
    ...
    const resolved = await this.dashboardRepository.resolveNodes(search).catch((err) => {
      console.error('[DashboardService] resolveNodes error:', err.message)
    ...
    let weatherRow, rainRows, riskRows, alertRows, tempHumRows, riskTrendRows

    if (isGlobal) {
      [weatherRow, rainRows, riskRows, alertRows, tempHumRows, riskTrendRows] = await Promise.all([
        this.dashboardRepository.getCurrentWeather(weatherNodeIds, true, h).catch(() => null),
        this.dashboardRepository.getRainForecast(weatherNodeIds, true, predictionNodeIds, h).catch(() => []),
        this.dashboardRepository.getCurrentFloodRiskCounts(h, true, predictionNodeIds).catch(() => []),
        this.dashboardRepository.getRecentAlerts(10).catch(() => []),
        this.dashboardRepository.getTempHumidity(weatherNodeIds, true, h).catch(() => []),
        this.dashboardRepository.getRiskTrend(predictionNodeIds, true, h <= 48 ? h : 168).catch(() => []),
      ])
    } else {
      let hourlyData;
      [weatherRow, riskRows, alertRows, riskTrendRows, hourlyData] = await Promise.all([
        this.dashboardRepository.getCurrentWeather(weatherNodeIds, false, h).catch(() => null),
        this.dashboardRepository.getCurrentFloodRiskCounts(h, false, predictionNodeIds).catch(() => []),
        this.dashboardRepository.getRecentAlerts(10).catch(() => []),
        this.dashboardRepository.getRiskTrend(predictionNodeIds, false, h <= 48 ? h : 168).catch(() => []),
        this.dashboardRepository.getLocationHourlyData(search, h).catch(() => [])
      ])
      
      // Merge hourlyData thành rainRows và tempHumRows để tương thích Frontend cũ
      rainRows = hourlyData;
      tempHumRows = hourlyData;
      
      // Lấy thời tiết hiện tại từ array nếu không có weatherRow
      if ((!weatherRow || weatherRow.temp === null) && hourlyData.length > 0) {
        const current = hourlyData[0];
        weatherRow = { temp: current.temp, rhum: current.rhum, wspd: current.wspd };
      }
    }
