...
export function MainLayout() {
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-slate-50 dark:bg-slate-950">
      <aside className="fps-card flex w-64 flex-shrink-0 flex-col overflow-y-auto p-4">
            ...
            <div className="flex items-center justify-between gap-3 rounded-xl bg-slate-50 px-3 py-2 dark:bg-slate-950/40">
              <div className="flex items-center gap-2 min-w-0">
                {user?.avatar_url ? (
                  ...
                  <div className="grid h-9 w-9 place-items-center rounded-xl bg-white ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-800">
                    <UserCircle2 className="h-5 w-5 text-slate-500 dark:text-slate-300" />
                  </div>
                )}
                <div className="min-w-0">
                  <div className="truncate text-xs font-semibold text-slate-900 dark:text-slate-100">{user?.full_name ?? '-'}</div>
              ...
              <button
                type="button"
                onClick={() => {
                  // Logout thật: clear token + state rồi điều hướng về /login
                  logout()
                  navigate('/login')
                }}
