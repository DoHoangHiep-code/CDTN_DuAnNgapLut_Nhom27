...
router.get('/flood-prediction/nodes/:id/current', async (req, res, next) => {
  try {
    ...
    return res.status(200).json({
      ...
      data: {
        node_id: nodeIdStr,
        location_name,
        flood: predRows && predRows.length > 0 ? {
          flood_depth_cm: predRows[0].flood_depth_cm,
          risk_level:     predRows[0].risk_level,
          explanation:    predRows[0].explanation,
          time:           predRows[0].time,
        } : null,
        weather: weatherData,
      }
    })
  ...
  try {
    ...
    if (!nearestRows || nearestRows.length === 0) {
      return res.status(404).json({
        success: false,
        error: { message: 'Khu vực này nằm ngoài vùng hỗ trợ dự báo.' },
      })
    }
    ...
    const predTime = predRows && predRows.length > 0 ? predRows[0].time : new Date()
    const weather = await inferWeatherForNode(nearestNode, predTime).catch(() => null)
